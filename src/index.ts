...
function resolveScriptsDir(pluginRoot: string): string {
  const candidates = [
...
function resolveAgentsDir(pluginRoot: string): string {
  const candidates = [
...
function resolveCommandsDir(pluginRoot: string): string {
  const candidates = [
...
function parseFrontmatterMd(content: string): { frontmatter: Record<string, string>; body: string } {
  const match = content.match(/^---\n([\s\S]*?)\n---\n([\s\S]*)$/);
...
let _skillsDirCache: string | undefined = undefined;
let _pluginRootCache: string | undefined = undefined;
...
const orchestraTasksTool = tool({
  description:
    "Manage orchestra tasks.jsonl – shared protocol across harnesses. Read, append, update-status, list DAG. Handles sequential IDs T001..., collision renumber, and parses .progress files.",
  args: {
    ...
    try {
      if (args.action === "read" || args.action === "list" || args.action === "validate") {
        ...
        if (args.action === "validate") {
          ...
          if (dup.length) return `Validation WARNING: duplicate IDs ${[...new Set(dup)].join(",")}. Use renumber-tasks.py to fix.`;
        ...
        for (const t of filtered) statusCounts[t.status] = (statusCounts[t.status] || 0) + 1;
      ...
      if (args.action === "append") {
        if (!args.task_json) return "append requires task_json arg";
        ensureDir();
        let newTasks: any[];
        try {
      ...
      if (args.action === "update-status") {
        if (!args.task_json) return "update-status requires task_json {id,status}";
        let payload: any;
        try {
        ...
        if (!id || !status) return "Need id and status";

        if (!fs.existsSync(resolvedTasksFile)) return `tasks.jsonl not found at ${resolvedTasksFile}`;
      ...
      return `Error in orchestra_tasks tool: ${(e as Error).message}`;
    }
  },
});
  ...
  async execute(args, ctx) {
    ...
    if (fs.existsSync(dagScript)) {
      try {
        ...
        if (err) return `DAG script stderr: ${err}`;
      } catch (e) {
        // fallback
      }
    }
...
export const OrchestraPlugin: Plugin = async ({ client, directory }) => {
  ...
  const shouldHandle = (file: string) => {
    if (!file.includes("docs/mvp") && !file.includes(".progress") && !file.includes("tasks.jsonl")) return false;
    const now = Date.now();
    const lastOwn = ownWrites.get(file) || 0;
    if (now - lastOwn < DEBOUNCE_MS * 2) return false; // skip own write
      ...
      try {
        ...
        if (fs.existsSync(agentsDirResolved)) {
          ...
          for (const file of fs.readdirSync(agentsDirResolved)) {
            ...
            if (frontmatter.description) config.agent[agentName].description = frontmatter.description;
            if (frontmatter.mode) config.agent[agentName].mode = frontmatter.mode;
            if (frontmatter.color) config.agent[agentName].color = frontmatter.color;
            if (frontmatter.hidden) config.agent[agentName].hidden = frontmatter.hidden === "true";
            // Prompt
      ...
      try {
        ...
        const featureSummaries = Object.entries(byFeature)
          .map(([feat, ts]) => {
            ...
            const ready = ts.filter((t) => t.status === "open" && (t.dependencies || []).every((d: string) => tasks.find((x) => x.id === d)?.status === "closed")).length;
...
If resuming, re-invoke executing-tasks skill with feature name – orchestrator picks up non-closed tasks. Reset stale in_progress → open before launch.`);
        ...
        output.context.push(`## Orchestra Context Error\nFailed to read tasks.jsonl: ${(e as Error).message}`);
      }
    },
      ...
      if (event.type === "file.watcher.updated") {
        ...
        if (file.endsWith("tasks.jsonl") && fsEvent === "change") {
          try {
            ...
            const counts: Record<string, number> = {};
            for (const t of tasks as any[]) counts[t.status] = (counts[t.status] || 0) + 1;
            await client.app.log({
              body: {
                service: "orchestra-watcher",
                level: "info",
